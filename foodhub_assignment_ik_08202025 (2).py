...
# Import Imput File
food_hub_eda = pd.read_csv(
    "/content/drive/MyDrive/foodhub_order.csv", na_values=["Not given"]
)
        ...
        .replace(
            {
                ...
                "Joe's Shanghai _Àü£¾÷´": "Joe's Shanghai",
                "Big Wong Restaurant _¤¾Ñ¼": "Big Wong Restaurant",
                "'wichcraft": "Wichcraft",
...
unique_restaurants = (
    food_hub_eda[["restaurant_name", "restaurant_name_clean"]]
    .drop_duplicates()
    .reset_index(drop=True)
)
print(
    f"\nTable 5: Data Preprocessing – Original Versus Standardized Restaurant Names "
    f"(N={len(unique_restaurants)})\n"
)
...
counts = pd.crosstab(
    df["restaurant_name_clean"], df["day_of_the_week"], dropna=False, margins=False
)
...
food_hub_eda["rating_new"] = food_hub_eda["rating"].fillna(
    food_hub_eda.groupby(["restaurant_name_clean", "day_of_the_week"])[
        "rating"
...
sns.heatmap(
    data=food_hub_eda[
        ["food_preparation_time", "cost_of_the_order", "delivery_time"]
    ].corr(),
    annot=True,
    cmap="YlGnBu",
    vmin=-1,
